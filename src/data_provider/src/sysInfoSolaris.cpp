...
void SysInfo::getProcessesInfo(std::function<void(nlohmann::json &)> callback) const
{
    ...
    for (const auto &procfile : procfiles)
    {
        ...
        std::ifstream ifInfo{WM_SYS_PROC_DIR + std::string(procfile) + "/psinfo", std::ios::binary};
        std::ifstream ifStatus{WM_SYS_PROC_DIR + std::string(procfile) + "/status", std::ios::binary};
        std::ifstream ifCred{WM_SYS_PROC_DIR + std::string(procfile) + "/cred", std::ios::binary};

        const auto procWrapper{std::make_shared<ProcessSolarisInterface>(ifInfo, ifStatus, ifCred)};
